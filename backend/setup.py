...
def create_virtualenv():
    if not os.path.exists('backend/env'):
        ...
        result = subprocess.call([sys.executable, '-m', 'venv', 'backend/env'])
        if result == 0:
...
def install_python_dependencies():
    ...
    if os.name == 'nt':
        pip_executable = os.path.join('backend', 'env', 'Scripts', 'pip.exe')
    else:
        pip_executable = os.path.join('backend', 'env', 'bin', 'pip')
    ...
    if os.path.exists(pip_executable):
        ...
        result = subprocess.call(
            [pip_executable, 'install', '-r', 'backend/requirements.txt'])
        if result == 0:
